...
class TR3DHead(nn.Module):
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=dict(nms_pre=50, iou_thr=.5, score_thr=.01)):
        super(TR3DHead, self).__init__()
        self.voxel_size = voxel_size
        self.assigner = TR3DAssigner(top_pts_threshold=32, label2level=[0])
        self.bbox_loss = AxisAlignedIoULoss(mode='diou', reduction='none')
    ...
    def _loss(self, bbox_preds, cls_preds, points,
        ...
        for i in range(len(img_metas)):
            ...
            #     print(len(img_metas), len(bbox_preds[0]))
                # exit()
    ...
    def forward_train(self, x, gt_bboxes=None, gt_labels=None, img_metas=None):
